...
Future<void> main(List<String> args) async {
  ...
  if (await _handleAccessPasswordConfiguration(args)) {
    return;
  }
  // main window
...
Future<bool> _handleAccessPasswordConfiguration(List<String> args) async {
  ...
  final expectedApproveMode = password.isEmpty ? 'click' : 'password';
  final expectedVerificationMethod =
      password.isEmpty ? null : 'use-permanent-password';
  ...
  try {
    ...
    final savedApproveMode = await bind.mainGetOption(key: kOptionApproveMode);
    if (savedPassword != password ||
      ...
      throw StateError(
        ...
        'expectedVerificationMethod=$expectedVerificationMethod, '
        'savedApproveMode=$savedApproveMode, '
        'expectedApproveMode=$expectedApproveMode',
      );
    }
    exit(0);
  } catch (error, stackTrace) {
    stderr.writeln('configure access password failed: $error');
