...
def make_parser():
    ...
    parser.add_argument(
        ...
        type=str,
        default="rustdesk",
        help="Internal app identifier used for protocol and registry keys.",
    )
    parser.add_argument(
        "--exe-name",
        type=str,
        default="rustdesk",
        help="Executable base name without extension.",
    )
    parser.add_argument(
        "--service-name",
        type=str,
        default="RustDesk",
        help="Windows service name.",
    )
    parser.add_argument(
        "--install-dir-name",
        type=str,
        default="",
        help="Install directory name. Defaults to app name.",
    )
    parser.add_argument(
        "--config-name",
        type=str,
        default="",
        help="Config directory name. Defaults to app name.",
    )
    parser.add_argument(
...
def read_lines_and_start_index(file_path, tag_start, tag_end):
    ...
    if index_end <= index_start:
        print(f'Error: end tag "{tag_end}" must appear after start tag "{tag_start}"')
        return None, None, None
...
def gen_pre_vars(args, dist_dir):
    def func(lines, index_start):
        install_dir_name = args.install_dir_name or args.app_name
        config_name = args.config_name or args.app_name
        upgrade_code = uuid.uuid5(uuid.NAMESPACE_OID, args.exe_name + ".exe")
        ...
        to_insert_lines = [
            ...
            f'{indent}<?define ProductLower="{args.app_id.lower()}" ?>\n',
            f'{indent}<?define ExecutableName="{args.exe_name}" ?>\n',
            f'{indent}<?define ServiceName="{args.service_name}" ?>\n',
            f'{indent}<?define InstallDirName="{install_dir_name}" ?>\n',
            f'{indent}<?define ConfigName="{config_name}" ?>\n',
            f'{indent}<?define UriScheme="{args.app_id.lower()}" ?>\n',
            f'{indent}<?define RuntimeBrokerName="RuntimeBroker_{args.exe_name}.exe" ?>\n',
...
def replace_rustdesk_tokens(text, app_name):
    if app_name == "RustDesk":
        return text

    if app_name.endswith("RustDesk"):
        prefix = app_name[: -len("RustDesk")]
        if prefix:
            text = re.sub(
                rf"(?:{re.escape(prefix)}){{2,}}RustDesk\b",
                app_name,
                text,
            )
            text = re.sub(
                rf"(?<!{re.escape(prefix)})\bRustDesk\b",
                app_name,
                text,
            )
            return text

    return text.replace("RustDesk", app_name)
...
def replace_app_name_in_langs(app_name):
    ...
    for file_path in langs_dir.glob("*.wxl"):
        with open(file_path, "r", encoding="utf-8") as f:
            lines = f.readlines()
        for i, line in enumerate(lines):
            lines[i] = replace_rustdesk_tokens(line, app_name)
...
def replace_app_name_in_custom_actions(app_name):
    ...
    for file_path in chain(custion_actions_dir.glob("*.cpp"), custion_actions_dir.glob("*.h")):
        with open(file_path, "r", encoding="utf-8") as f:
            lines = f.readlines()
        for i, line in enumerate(lines):
            line = replace_rustdesk_tokens(line, app_name)
...
def gen_custom_ARPSYSTEMCOMPONENT_True(args, dist_dir):
    def func(lines, index_start):
        ...
        lines_new.append(
            f'{indent}<RegistryValue Type="string" Name="DisplayIcon" Value="[INSTALLFOLDER_INNER]{args.exe_name}.exe" />\n'
        )
        lines_new.append(
...
def init_global_vars(dist_dir, app_name, args):
    dist_app = dist_dir.joinpath(args.exe_name + ".exe")
...
def update_license_file(app_name):
    ...
    license_content = replace_rustdesk_tokens(license_content, app_name)
...
if __name__ == "__main__":
    ...
    if not gen_auto_component(args.exe_name, dist_dir):
        sys.exit(-1)
