...
pub fn start_os_service() {
    if let Err(e) =
        windows_service::service_dispatcher::start(crate::get_service_name(), ffi_service_main)
    {
...
async fn run_service(_arguments: Vec<OsString>) -> ResultType<()> {
    ...
    let status_handle =
        service_control_handler::register(crate::get_service_name(), event_handler)?;
...
fn get_default_install_path() -> String {
    ...
    format!("{}\\{}", pf, crate::get_install_dir_name())
}
...
fn get_install_info_with_subkey(subkey: String) -> (String, String, String, String) {
    ...
    let exe = format!("{}\\{}", path, crate::get_executable_name());
...
pub fn rename_exe_cmd(src_exe: &str, path: &str) -> ResultType<String> {
    let src_exe_filename = PathBuf::from(src_exe)
        ...
        .to_string_lossy()
        .to_string();
    let exe_name = crate::get_executable_name().to_lowercase();
...
",
    app_name = crate::get_app_name(),
    service_name = crate::get_service_name(),
...
pub fn is_self_service_running() -> bool {
    is_service_running(&crate::get_service_name())
}
...
pub fn try_kill_rustdesk_main_window_process() -> ResultType<()> {
    // Kill the main GUI process without extra args, should only be called by --server.
    // We can find the exact process which occupies the ipc, see more from https://github.com/winsiderss/systeminformer
    let app_name = crate::get_executable_basename().to_lowercase();
