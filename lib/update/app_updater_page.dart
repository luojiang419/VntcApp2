...
class AppUpdaterPage extends StatefulWidget {
  ...
  final AppUpdateInstallSession session;
  ...
  Future<void> _run() async {
    try {
      ...
      setState(() {
        ...
        _substep = '请重新打开 VNTC APP 后在设置页重试。';
      });
    }
  }
                  ...
                  Expanded(
                    child: Text(
                      _isError
                          ...
                          : _isSuccess
                              ? '新版本已启动，更新窗口即将自动关闭。'
                              : '请勿关闭该窗口，安装过程会自动完成并重新打开 VNTC APP。',
                      style: TextStyle(
...
class _StepChip extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return AnimatedContainer(
      ...
      decoration: BoxDecoration(
        color:
            active ? accent.withValues(alpha: 0.12) : const Color(0xFF20242A),
        borderRadius: BorderRadius.circular(8),
        border: Border.all(
          color:
              active ? accent.withValues(alpha: 0.32) : const Color(0xFF30343A),
        ),
      ),
      child: Text(
        '${index + 1}. $label',
        style: TextStyle(
          color: active ? const Color(0xFFE8EAED) : const Color(0xFF9AA1AA),
          fontSize: 12,
