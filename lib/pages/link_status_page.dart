...
class _LinkStatusPageState extends State<LinkStatusPage>
  ...
  bool _remoteAssistExpanded = false;
  ...
  Future<void> _repairRemoteAssist() async {
    try {
      await _remoteAssistManager.repair();
      if (!mounted) {
        return;
      }
      showTopToast(context, '已执行远程协助修复', isSuccess: true);
    } catch (error) {
      if (!mounted) {
        return;
      }
      showTopToast(context, '修复失败: $error', isSuccess: false);
    }
  }

  Widget _buildRemoteAssistPanel(
    bool isDark,
      ...
      child: Column(
        children: [
          InkWell(
            onTap: () {
              setState(() {
                _remoteAssistExpanded = !_remoteAssistExpanded;
              });
            },
            borderRadius: BorderRadius.circular(context.cardRadius),
            child: Padding(
  ...
  Color _remoteAssistStatusColor(RemoteAssistHealthStatus health) {
    ...
    if (health.runtimeAvailable || health.serviceInstalled) {
      return Colors.orange;
    }
    return Colors.redAccent;
  }
  ...
  String _remoteAssistStatusLabel(RemoteAssistHealthStatus health) {
    ...
    if (health.runtimeAvailable || health.serviceInstalled) {
      return '远程协助可检测但仍有待处理项，点击展开查看';
    }
    if (health.bundledRepairAvailable) {
      return '远程协助未就绪，可点击展开后安装或修复';
    }
    return '远程协助未就绪，点击展开查看当前检测结果';
  }

  Widget _buildRemoteAssistChip(
    bool isDark, {
  ...
  Widget _buildRemoteAssistInfoRow(bool isDark, String label, String value) {
    return Padding(
